...
本プロジェクトは**Model Context Protocol (MCP) サーバー**として実装されており、主にAIエディタ（Cursor、Claude Desktop等）やAIエージェントからツールとして呼び出されることを前提としています。
...
⚠️ **タイムアウト注意**: Cursorでは内部タイムアウトが設定されており、長時間の処理でタイムアウトが発生する可能性があります。タイムアウトが発生した場合も、バックグラウンドで処理は継続されます。
